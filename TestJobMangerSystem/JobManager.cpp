...
JobManager::CWorkerBackEndProfiler::~CWorkerBackEndProfiler()
{
	if (m_WorkerStatsInfo.m_pWorkerStats)
		_aligned_free(m_WorkerStatsInfo.m_pWorkerStats);
}

///////////////////////////////////////////////////////////////////////////////
void JobManager::CWorkerBackEndProfiler::Init(unsigned short numWorkers)
{
	// Init Job Stats
	...
	m_WorkerStatsInfo.m_pWorkerStats = (JobManager::SWorkerStats*)_aligned_malloc(nWorkerStatsBufSize, 128);
...
void JobManager::CWorkerBackEndProfiler::Update(const UINT32 curTimeSample)
{
	...
	unsigned char nNextIndex = (m_nCurBufIndex + 1);
...
{
	unsigned char nTailIndex = (m_nCurBufIndex + 1);
...
void JobManager::CWorkerBackEndProfiler::GetFrameStatsSummary(SWorkerFrameStatsSummary& rStats) const
{
	...
	int nSamplePeriod = abs((int)m_WorkerStatsInfo.m_nEndTime[nTailIndex] - (int)m_WorkerStatsInfo.m_nStartTime[nTailIndex]);
	...
	for (unsigned char i = 0; i < m_WorkerStatsInfo.m_nNumWorkers; ++i)
	{
	...
	rStats.nNumActiveWorkers = (unsigned char)m_WorkerStatsInfo.m_nNumWorkers;
...
void JobManager::CWorkerBackEndProfiler::GetFrameStatsSummary(SJobFrameStatsSummary& rStats) const
{
	...
	unsigned short totalIndividualJobCount = 0;
	UINT32 totalJobsExecutionTime = 0;
	UINT32 totalJobCount = 0;
	for (unsigned short i = 0; i < JobManager::detail::eJOB_FRAME_STATS_MAX_SUPP_JOBS; ++i)
	{
...
{
	assert(jobId < JobManager::detail::eJOB_FRAME_STATS_MAX_SUPP_JOBS);/*,
...
*/
	for (unsigned char i = 0; i < JobManager::detail::eJOB_FRAME_STATS; ++i)
		m_JobStatsInfo.m_pJobStats[(i* JobManager::detail::eJOB_FRAME_STATS_MAX_SUPP_JOBS) +jobId].cpName = jobName;
}

///////////////////////////////////////////////////////////////////////////////
void JobManager::CWorkerBackEndProfiler::RecordJob(const unsigned short profileIndex, const unsigned char workerId, const UINT32 jobId, const UINT32 runTimeMicroSec)
{
	assert(workerId < m_WorkerStatsInfo.m_nNumWorkers);/*, string().Format("JobManager::CWorkerBackEndProfiler::RecordJob: workerId is out of scope. workerId:%u , scope:%u"
...
///////////////////////////////////////////////////////////////////////////////
unsigned short JobManager::CWorkerBackEndProfiler::GetProfileIndex() const
{
...
///////////////////////////////////////////////////////////////////////////////
void JobManager::CWorkerBackEndProfiler::GetWorkerStats(const unsigned char nBufferIndex, JobManager::CWorkerFrameStats& rWorkerStats) const
{
	assert(rWorkerStats.numWorkers <= m_WorkerStatsInfo.m_nNumWorkers);

	// Calculate percentage range multiplier for this frame
	// Take absolute time delta to handle microsecond time sample counter overfilling
	UINT32 nSamplePeriode = std::max(m_WorkerStatsInfo.m_nEndTime[nBufferIndex] - m_WorkerStatsInfo.m_nStartTime[nBufferIndex], (UINT32)1);
	...
	rWorkerStats.numWorkers = (unsigned char)m_WorkerStatsInfo.m_nNumWorkers;
...
///////////////////////////////////////////////////////////////////////////////
void JobManager::CWorkerBackEndProfiler::GetJobStats(const unsigned char nBufferIndex, TJobFrameStatsContainer& rJobStatsContainer, IWorkerBackEndProfiler::EJobSortOrder jobSortOrder) const
{
...
///////////////////////////////////////////////////////////////////////////////
void JobManager::CWorkerBackEndProfiler::ResetWorkerStats(const unsigned char nBufferIndex, const UINT32 curTimeSample)
{
...
///////////////////////////////////////////////////////////////////////////////
void JobManager::CWorkerBackEndProfiler::ResetJobStats(const unsigned char nBufferIndex)
{
...
{
#if defined(JOBMANAGER_SUPPORT_PROFILING)
	SJobProfilingData* pJobProfilingData = GetJobManagerInterface()->GetProfilingData(rJobState.nProfilerIndex);
	pJobProfilingData->nWaitBegin = gEnv->pTimer->GetAsyncTime();
	pJobProfilingData->nThreadId = GetCurrentThreadId();
#endif
...
//	color.r = static_cast<unsigned char>(hash);
//	color.g = static_cast<unsigned char>(hash >> 8);
//	color.b = static_cast<unsigned char>(hash >> 16);
//
//	return color;
//}
...
void JobManager::CJobManager::AddJob(JobManager::CJobDelegator& crJob, const JobManager::TJobHandle cJobHandle)
{
	...
	const unsigned char cParamSize = cOrigParamSize >> 4;
...
#if defined(JOBMANAGER_SUPPORT_PROFILING)
	SJobProfilingData* pJobProfilingData = GetJobManagerInterface()->GetProfilingData(infoBlock.profilerIndex);
...

{
	...
	for (WORD i = 0; i < frames; ++i) {
		...
		if (SymFromAddr(process, address, &displacementSym, pSymbol)
			...
			CStringA temp;
			temp.Format("%s%s%s%s%s%d%s%x%s\n", "\t", pSymbol->Name, " at ", line.FileName, ":", line.LineNumber, "(0x", pSymbol->Address,  ")");
			result += temp;
		}
...
JobManager::SJobProfilingData* JobManager::CJobManager::GetProfilingData(unsigned short nProfilerIndex)
{
#if defined(JOBMANAGER_SUPPORT_PROFILING)
...
unsigned short JobManager::CJobManager::ReserveProfilingData()
{
#if defined(JOBMANAGER_SUPPORT_PROFILING)
	...
	unsigned short nProfilerIndex = (nFrameIdx << 14) | nProfilingDataEntryIndex;
...
//	{
//		return strcmp(pName.GetBuffer(0), rOther.pName.GetBuffer(0)) == 0;
//	}
//	bool operator<(const SRegionTime& rOther) const
//	{
//		return strcmp(pName.GetBuffer(0), rOther.pName.GetBuffer(0)) < 0;
...
//		return strcmp(rA.pName.GetBuffer(0), rB.pName.GetBuffer(0)) < 0;
//	}
//};
//
...
void JobManager::CJobManager::Update(int nJobSystemProfiler)
{
	...
	// get used thread ids
	unsigned long nMainThreadId, nRenderThreadId;
...
#if defined(JOBMANAGER_SUPPORT_PROFILING)
	// get used thread ids
	static unsigned long nMainThreadId = ~0;
	static unsigned long nRenderThreadId = ~0;
...
//	{
//		m_RegionColors[marker] = GenerateColorBasedOnName(marker.GetBuffer(0));
//	}
