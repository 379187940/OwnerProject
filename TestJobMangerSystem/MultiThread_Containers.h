...
inline void SingleProducerSingleConsumerQueue<T >::Init(size_t nSize)
{
	...
	m_arrBuffer = alias_cast<T*>(_aligned_malloc(nSize * sizeof(T), 16));
	m_nBufferSize = nSize;
}
...
private:
	T* m_arrBuffer;
	volatile unsigned int* m_arrStates;
	unsigned int m_nBufferSize;

	volatile unsigned int m_nProducerIndex;
	volatile unsigned int m_nComsumerIndex;
	volatile unsigned int m_nRunning;
	volatile unsigned int m_nProducerCount;
};
...
{
	_aligned_free(m_arrBuffer);
	_aligned_free((void*)m_arrStates);
	m_nBufferSize = 0;
}
...
inline void N_ProducerSingleConsumerQueue<T >::Init(size_t nSize)
{
	...
	m_arrBuffer = alias_cast<T*>(_aligned_malloc(nSize * sizeof(T), 16));
	m_arrStates = alias_cast<unsigned int*>(_aligned_malloc(nSize * sizeof(unsigned int), 16));
	memset((void*)m_arrStates, 0, sizeof(unsigned int) * nSize);
	m_nBufferSize = nSize;
}
