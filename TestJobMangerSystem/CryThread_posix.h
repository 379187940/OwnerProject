...
private:
	template<typename T>
	bool TimedWait_Internal(T& lock, unsigned int millis)
	{
		struct timeval now;
		...
		while (true)
		{
			...
			unsigned long long nsec = (unsigned long long)now.tv_usec * 1000 + (unsigned long long)millis * 1000000;
			if (nsec >= 1000000000)
			{
...
public:
	...
	bool Wait(const unsigned int timeoutMillis);
