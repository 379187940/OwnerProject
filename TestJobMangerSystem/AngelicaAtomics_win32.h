...
inline LONG AngelicaInterlockedAdd(volatile int* pDst, int add)
{
	return AngelicaInterlockedAdd((volatile LONG*)(pDst), (LONG)add);
}
...
#else
	return AngelicaInterlockedExchangeAdd(pDst, add) + add;
#endif
}
...
inline size_t AngelicaInterlockedAdd(volatile size_t* pDst, size_t add)
{
	...
	return (size_t)AngelicaInterlockedAdd((volatile LONG*)pDst, (LONG)add);

#endif
}

// Returns initial value prior exchange
inline LONG AngelicaInterlockedExchange(volatile int* pDst, int exchange)
{
	return AngelicaInterlockedExchange((volatile LONG*)pDst, (LONG)exchange);
}
...
#else
	static_assert(sizeof(size_t) == sizeof(LONG), "Unsecured cast. size_t is not same size as LONG.");
	return (size_t)AngelicaInterlockedExchangeAdd((volatile LONG*)pDst, (LONG)add); // intrinsic returns previous value
#endif
}
...
// Returns initial address prior exchange
inline LONG AngelicaInterlockedCompareExchange(LONG volatile* pDst, LONG exchange, LONG comperand)
{
...
// Returns initial value prior exchange
inline long long AngelicaInterlockedCompareExchange64(volatile long long* pDst, long long exchange, long long compare)
{
